...
| 2026-05-08 | `schema_version=1` (revisão 0.4 — Sprint D3 implementado) | `/sprint D3` (agente implementador) | **Confirmação de conformidade entre o contrato especificado e a implementação concreta da Sprint D3.** Mudanças: (1) **D3.1 — `occ_run` real** em `scripts/lib/occ_bridge.sh`: implementação completa com validação de blocklist/allowlist (exit 100), verificação de container rodando (exit 14), aquisição de `client_lock` para comandos state-changing (exit 17), execução via `docker exec` com array para prevenir injeção de argumentos, senha via `NEXTCLOUD_USER_PASSWORD` env (nunca em argv), `timeout` 60s (exit 15), parse de saída JSON para subcomandos `OCC_JSON_CAPABLE` em `parsed_result`, audit log NDJSON via `audit_occ`. (2) **D3.2 — inbox-staging** em `scripts/lib/job_queue.sh`: funções `inbox_metadata_create/get/consume/delete` (Redis hash `nc:inbox:<staging_id>`), `inbox_staging_consume` com validação de tamanho por arquivo (5 MB, exit 18) e total (10 MB, exit 18); `store_pending_pw`/`read_and_clear_pending_pw` para senha efêmera `nc:pending_pw:<job_id>` com TTL 300s. GC atualizado para remover diretórios inbox não-vazios após 24h. (3) **D3.3 / D3.4 — user/group/apps lifecycle** em `scripts/lib/feature_o.sh` e `scripts/worker.sh`: `cmd_user_create`, `cmd_user_remove`, `cmd_user_modify`, `cmd_group_create`, `cmd_group_remove`, `cmd_group_modify`, `cmd_apps_enable`, `cmd_apps_disable` (enqueue-only em `manage.sh`); `worker_exec_user_*`, `worker_exec_group_*`, `worker_exec_apps_*` (execução real via `occ_run` no worker). Rota de despacho dupla no worker: Feature O direto para `worker_exec_feature_o`; legado via `run_job`. (4) **D3.5 — create estendido** em `scripts/lib/feature_o_ext.sh`: `cmd_create_post_extended` com `--apps=<csv>` (app:install/enable), `--full-apps` (app:enable pré-definidos), `--staging-id` (branding via `docker cp` + `occ theming:config`). (5) **D3.6 — remove estendido**: `--confirm=<cliente>` obrigatório em sync quando não `--force`; `--backup-first` enfileira job `backup-then-remove` via `cmd_backup_then_remove_enqueue`; `cmd_backup_then_remove` no worker executa `cmd_backup` sequido de `cmd_remove`. (6) **D3.7 — testes de integração** (bats): 3 novas suítes (`test_occ_bridge.bats` 14 testes, `test_inbox_staging.bats` 8 testes, `test_feature_o.bats` 20 testes); testes D2 `test_manage_async_dispatch.bats` (tests 61–63) atualizados para refletir comandos Feature O agora funcionais. Total: 138 testes passando. **`schema_version` permanece "1"**: implementação D3 é conformante com o contrato especificado em revisões anteriores; nenhuma mudança breaking. |
